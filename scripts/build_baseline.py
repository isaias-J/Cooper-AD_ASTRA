...
from codefest.core import clean_text, normalized_rel, phenomenon, stable_doc_id, write_jsonl
...
def main():
 p=argparse.ArgumentParser(); p.add_argument("--corpus-root",type=Path,required=True); p.add_argument("--model",default="intfloat/multilingual-e5-base"); p.add_argument("--target-tokens",type=int,default=360); p.add_argument("--max-tokens",type=int,default=480); p.add_argument("--batch-size",type=int,default=32); p.add_argument("--audit-only",action="store_true"); a=p.parse_args()
 ...
 for source in sorted(x for x in root.rglob("*") if x.is_file() and x.name != ".DS_Store"):
  ...
  try:
   extracted=extract(source); text=extracted["text"]
   if not text: raise ValueError("Extraccion sin texto")
   for position,body in enumerate(chunk_text(text,encoder.tokenizer,a.target_tokens,a.max_tokens)):
    chunks.append({"doc_id":doc_id,"chunk_id":f"{doc_id}-chunk-{position:04d}","fuente":rel,"formato":source.suffix.lower().lstrip("."),"fenomeno":phenomenon(rel),"posicion":position,"num_tokens":len(encoder.tokenizer.encode(body,add_special_tokens=False)),"texto":body,"ruta_relativa":rel,**extracted["metadata"]})
